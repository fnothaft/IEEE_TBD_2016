...
However, with Spark's Scala API in Kira SE, neither the jFITS library nor the JNI are thread safe. 
...
We migrated the previous version of Kira that used Spark's Scala interface~\cite{zhang15} to PySpark
...
With Kira-SE-v2, the slowdown is caused by coordination between the Scala, Python and C runtimes.
...
2310 image files where each image is $\sim$6MB.
...
shrinks as the core count increases. The performance of Kira-SE-v2 converges to the C performance beyond four cores.
The scaling curves indicate that the performance of the approaches begins to converge
as core count increases. At convergence, all approaches saturate the local
disk bandwidth.
...
implementation. Although Kira-SE-v1 has 2.2$\times$ worse performance than the C implementation when running on
...
on large clusters. 
\zhaonote{revised with Kira-SE-v2 performance}
Kira-SE-v2 combines improved locality with native C
performance, so we expect Kira-SE-v2 to be faster than the C implementation
at all scales.
...
Kira-SE-v1 is 2.7$times$ slower than the C version on eight cores. 
...
2.2$\times$--3.1$\times$ across scales.
Both Kira-SE-v1 and Kira-SE-v2 achieve near linear scalability.
...
mode. When compared to the \emph{distributed} mode, Kira-SE-v1 delivers comparable performance, as it is 18\% slower. 
In our experiments with the 1TB dataset in Section~\ref{sec:1TB-EC2}, 
Kira-SE-v1 outperforms the \emph{distributed} mode.

\zhaonote{revised with Kira-SE-v2 performance}
As expected, Kira-SE-v2 outperforms the \emph{distributed} mode by 2.1$\times$ and 1.8$\times$
on 128 cores and 256 cores, respectively.
...
\subsubsection{Supercomputer Performance}
...
767.4~seconds and a standard deviation of 10.9~seconds. The stable performance of both versions of Kira SE
...
\subsection{Solid State Disk Performance}
...
with each arrival having $\sim128$ files (780~MB of data per arrival).
...
However,  the estimated average total delay decreases by 17.5\% and
the throughput decreases dramatically by 50\% due to system scalability bottlenecks when moving from 2 arrivals per batch to 1 arrival per batch.
In other words, the total work amount with 1 arrival per batch is not enough to maintain high utilization
of all 128 cores. 

The performance of Kira-SE-v2's batch mode across 128 cores presented in~\S\ref{sec:Performance-scaleout} 
...
Thus a reasonable configuration space for the streaming interval on our 128 core cluster is 3--9~s. This trades off
between a sustained throughput range of 694--515.3~MB/s and 
...
size. Due to its superior data locality, our Spark-based implementation achieves a speedup of $2.2\times$--$4.1\times$ the equivalent C 
implementation running on GlusterFS. Performance varies with the dataset and cluster scale. 
...
By leveraging the Spark Streaming module, we were able to deploy Kira SE as a streaming application.
...
A flexible programming interface, rich dataflow support, task scheduling capacity, locality optimization, and built-in support for fault tolerance make Spark a 
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{pictures/oliverzahn.jpg}}]{Oliver Zahn}
