...
Using the same implementation of Kira SE, a 128-core EC2 cloud deployment with Spark Streaming can achieve a second-scale total delay with a 
...
introduces more system overhead compared to pure Spark.
...
but increase the average total delay.
On the other hand, decreasing the streaming interval to less than 3~s will lower the sustained throughput dramatically.
Thus the tradeoff space of streaming interval on this 128 core cluster is $\{3-9\}$~s with a sustained throughput
ranging from $\{694-515.3\}$~MB/s. 
The according average total delay is in the range of $\{4-11.9\}$~s.
...
Leverage the Spark Streaming module, we were able to deploy pyKira SE as a streaming application.
On a 128 core cluster, pyKira SE can processing streaming images with a second-scale average total delay and
