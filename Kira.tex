...
\IEEEcompsocthanksitem Frank~Austin~Nothaft, Evan~Sparks, David~A.~Patterson are with the AMPLab, UC Berkeley
...
Furthermore, we show that by leveraging software originally designed for big data infrastructure, Kira SE achieves a 1.8$\times$ speedup over  
the C implementation running on the NERSC Edison supercomputer.
...
\begin{IEEEkeywords}
Distributed Computing, Data Processing, Astronomy
\end{IEEEkeywords}}
...
\IEEEPARstart{D}{ramatic} dataset size increases have made data processing a major bottleneck for scientific research in many disciplines, such as astronomy, genomics, the social sciences, and neuroscience.
...
\subsection{Spark Streaming}
In this section, we evaluate the pyKira SE performance with the metric of average total delay that is composed of schedule delay and execution delay.
The schedule delay is incurred when the data is ready but system can not start processing immediately due to the unfinished previous batch.
The execution delay is the time-to-solution for the data in the current interval.
The overall delay is an indicator of how quickly the results can include the data in the current interval, thus it reflects the freshness of the results.

We seek to set the streaming interval long enough to finish the processing on the current data.
For this purpose, we use the 65GB dataset and 16 m2.4xlarge instances (128 cores in total).
The 65GB dataset is divided into 88 partitions as the data comes in 88 arrivals, with each arrival having $\sim128$ files (780~MB).

Figure~\ref{fig:stream} shows the measurement of the execution delay when pyKira SE processes every 8 arrivals, 4 arrivals, 2 arrivals, and 1 arrival.
We see that the starting batch is significantly slower than other batches due to the startup cost, e.g., load libraries into memory.
Decreasing the arrival count for each batch halves the execution delay except from 2 arrivals to 1 arrival.
...
The average execution delay is reported in the {\it Exe Delay} column in Table~\ref{tb:stream}.
Based on the measured execution delay, we set the streaming interval for each setting as $\ceil{avg+stdev}$.
For each streaming interval, we assume the data comes in with a uniform rate and use the time point
when all data in this interval is processed as the finishing time.
The average total delay can be estimated as the $avg(total\_delay_{first\_file} + total\_delay_{last\_file})$.
...
\begin{table}[h]
  ...
  Sustained Input Rate (InRate) and Sustained Output Throughput (OutTput) are in MB/second}
    \begin{small}
...
From the analysis in Table~\ref{tb:stream}, we can see that lowering the input rate decreases
the estimated average total delay and in turn enhance the result freshness. 
However, given the system capability, processing too less data can not make efficient use
of all machines in the cluster, e.g., lowering the input rate to half from 2 arrivals to 1 arrival
only achieves a 50\% efficiency compared to the 2 arrival case. 
Thus the sustained output throughput is decreased by a factor of two. 
In particular of the LSST real time processing requirement, the measured throughput (257.6~MB/s) is
below the requirement (330~MB/s).

With the LSST's requirement, the lowest total delay of pyKira SE on a 16 m2.4xlarge cluster is $\sim4$ seconds.
